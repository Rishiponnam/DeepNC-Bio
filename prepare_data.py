...
for dataset in datasets:
    ...
    fpath = 'data/' + dataset + '/'
    train_fold = json.load(open(fpath + "folds/train_fold.txt"))
    train_fold = [ee for e in train_fold for ee in e ]
    valid_fold = json.load(open(fpath + "folds/test_fold.txt"))
    ligands = json.load(open(fpath + "ligands.txt"), object_pairs_hook=OrderedDict)
    ...
    for option in options:
        ...
        with open('data/' + dataset + '_' + option + '.csv', 'w') as f:
...
for data_name in ['davis', 'kiba', 'allergy']:
    options = ['train', 'test']
    for option in options:
        df = pd.read_csv(f'data/{data_name}_{option}.csv')
...
features = []
smile_graph = {}  # Assuming you want to store smile graphs
with open('features.txt', 'w') as file:
    for smile in compound_iso_smiles:
        a, b, c = smile_to_graph(smile)  # Assuming smile_to_graph returns (a, b, c)
        features.append(b)
